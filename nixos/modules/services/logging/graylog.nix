...

{
  ...
  options = {
    ...
    services.graylog = {
      ...
      enable = mkEnableOption (mdDoc "Graylog");
      ...
      package = mkOption {
        ...
        description = mdDoc "Graylog package to use.";
      };

      user = mkOption {
        type = types.str;
        default = "graylog";
        description = mdDoc "User account under which graylog runs";
      };

      isMaster = mkOption {
        type = types.bool;
        default = true;
        description = mdDoc "Whether this is the master instance of your Graylog cluster";
      };

      nodeIdFile = mkOption {
        type = types.str;
        default = "/var/lib/graylog/server/node-id";
        description = mdDoc "Path of the file containing the graylog node-id";
      };
      ...
      rootUsername = mkOption {
        type = types.str;
        default = "admin";
        description = mdDoc "Name of the default administrator user";
      };
      ...
      elasticsearchHosts = mkOption {
        ...
        description = mdDoc "List of valid URIs of the http ports of your elastic nodes. If one or more of your elasticsearch hosts require authentication, include the credentials in each node URI that requires authentication";
      };

      messageJournalDir = mkOption {
        type = types.str;
        default = "/var/lib/graylog/data/journal";
        description = mdDoc "The directory which will be used to store the message journal. The directory must be exclusively used by Graylog and must not contain any other files than the ones created by Graylog itself";
      };

      mongodbUri = mkOption {
        type = types.str;
        default = "mongodb://localhost/graylog";
        description = mdDoc "MongoDB connection string. See http://docs.mongodb.org/manual/reference/connection-string/ for details";
      };

      extraConfig = mkOption {
        type = types.lines;
        default = "";
        description = mdDoc "Any other configuration options you might want to add";
      };

      plugins = mkOption {
        description = mdDoc "Extra graylog plugins";
        default = [ ];
