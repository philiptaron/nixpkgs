...
in {
  options.networking.ucarp = {
    enable = mkEnableOption (mdDoc "ucarp, userspace implementation of CARP");

    interface = mkOption {
      type = types.str;
      description = mdDoc "Network interface to bind to.";
      example = "eth0";
    };

    srcIp = mkOption {
      type = types.str;
      description = mdDoc "Source (real) IP address of this host.";
    };

    vhId = mkOption {
      type = types.ints.between 1 255;
      description = mdDoc "Virtual IP identifier shared between CARP hosts.";
      example = 1;
    };

    passwordFile = mkOption {
      type = types.str;
      description = mdDoc "File containing shared password between CARP hosts.";
    ...
    neutral = mkOption {
      type = types.bool;
      description = mdDoc "Do not run downscript at start if the host is the backup.";
      default = false;
    };

    addr = mkOption {
      type = types.str;
      description = mdDoc "Virtual shared IP address.";
    };

    advBase = mkOption {
      type = types.ints.unsigned;
      description = mdDoc "Advertisement frequency in seconds.";
      default = 1;
    };

    advSkew = mkOption {
      type = types.ints.unsigned;
      description = mdDoc "Advertisement skew in seconds.";
      default = 0;
    };
    ...
    deadratio = mkOption {
      type = types.ints.unsigned;
      description = mdDoc "Ratio to consider a host as dead.";
      default = 3;
    };

    shutdown = mkOption {
      type = types.bool;
      description = mdDoc "Call downscript at exit.";
      default = false;
    };

    ignoreIfState = mkOption {
      type = types.bool;
      description = mdDoc "Ignore interface state, e.g., down or no carrier.";
      default = false;
    };

    noMcast = mkOption {
      type = types.bool;
      description = mdDoc "Use broadcast instead of multicast advertisements.";
      default = false;
    };

    extraParam = mkOption {
      type = types.nullOr types.str;
      description = mdDoc "Extra parameter to pass to the up/down scripts.";
      default = null;
    };
  ...
  meta.maintainers = with maintainers; [ oxzi ];
}
