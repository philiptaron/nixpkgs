...
let
    ...
    options = {
      ...
      data = mkOption {
        ...
        description = mdDoc ''
          ...
          Use imports or pkgs.readFile if you don't want this data in your config file.
        '';
      };

      dnssec = mkEnableOption (mdDoc "DNSSEC");

      dnssecPolicy = {
        algorithm = mkOption {
          type = types.str;
          default = "RSASHA256";
          description = mdDoc "Which algorithm to use for DNSSEC";
        };
        keyttl = mkOption {
          type = types.str;
          default = "1h";
          description = mdDoc "TTL for dnssec records";
        };
        coverage = mkOption {
          type = types.str;
          default = "1y";
        ...
        zsk = mkOption {
          ...
          description = mdDoc "Key policy for zone signing keys";
        };
        ksk = mkOption {
          type = keyPolicy;
          ...
          description = mdDoc "Key policy for key signing keys";
        };
      };
  ...
  keyPolicy = types.submodule {
    options = {
      keySize = mkOption {
        type = types.int;
        description = mdDoc "Key size in bits";
      };
      prePublish = mkOption {
        type = types.str;
        description = mdDoc "How long in advance to publish new keys";
      };
      postPublish = mkOption {
        type = types.str;
        description = mdDoc "How long after deactivation to keep a key in the zone";
      };
      rollPeriod = mkOption {
        type = types.str;
        description = mdDoc "How frequently to change keys";
      };
    };
  };
...
in
{
  ...
  options.services.nsd = {
    ...
    enable = mkEnableOption (mdDoc "NSD authoritative DNS server");

    bind8Stats = mkEnableOption (mdDoc "BIND8 like statistics");
    ...
    roundRobin = mkEnableOption (mdDoc "round robin rotation of records");
    ...
    ratelimit = {
      ...
      enable = mkEnableOption (mdDoc "ratelimit capabilities");
    ...
    remoteControl = {
      ...
      enable = mkEnableOption (mdDoc "remote control via nsd-control");
  ...
  meta.maintainers = with maintainers; [ hrdinka ];
}
