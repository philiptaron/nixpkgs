...
let
  ...
  configFile = pkgs.writeText "NetworkManager.conf" (concatStringsSep "\n" [
    (mkSection "main" {
      plugins = "keyfile";
    ...
    (mkSection "keyfile" {
      unmanaged-devices =
        ...
        else concatStringsSep ";" cfg.unmanaged;
    })
    (mkSection "logging" {
  ...
  */
  ...
  ++ cfg.plugins
  ++ optionals (!delegateWireless && !enableIwd) [
    pkgs.wpa_supplicant
  ];
...
in
{
    ...
    networking.networkmanager = {
              ...
              check =
                ...
                assertMsg
                  (types.package.check p
                    ...
                    && isString p.networkManagerPlugin)
                  ''
      ...
      fccUnlockScripts = mkOption {
        type = types.listOf (types.submodule {
          options = {
            id = mkOption {
              type = types.str;
              description = mdDoc "vid:pid of either the PCI or USB vendor and product ID";
            };
            path = mkOption {
              type = types.path;
              description = mdDoc "Path to the unlock script";
            };
          };
        });
        default = [ ];
      ...
      ensureProfiles = {
        profiles = with types; mkOption {
          ...
          apply = (filterAttrsRecursive (n: v: v != { }));
          default = { };
          example = {
            home-wifi = {
              connection = {
  ...
  config = mkIf cfg.enable {
    ...
    // listToAttrs (imap1
      (i: s:
        {
          name = "NetworkManager/dispatcher.d/${dispatcherTypesSubdirMap.${s.type}}03userscript${fixedWidthNumber 4 i}";
    ...
    systemd.services.ModemManager = {
      ...
      path = optionals (cfg.fccUnlockScripts != []) [ pkgs.libqmi pkgs.libmbim ];
    };
    ...
    systemd.services.NetworkManager-ensure-profiles = mkIf (cfg.ensureProfiles.profiles != { }) {
      ...
      '' + concatMapStringsSep "\n"
        (profile: ''
          ${pkgs.envsubst}/bin/envsubst -i ${ini.generate (escapeShellArg profile.n) profile.v} > ${path (escapeShellArg profile.n)}
        '') (mapAttrsToList (n: v: { inherit n v; }) cfg.ensureProfiles.profiles)
      + ''
