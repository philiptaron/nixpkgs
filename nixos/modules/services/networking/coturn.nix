...
${optionalString cfg.lt-cred-mech "lt-cred-mech"}
${optionalString cfg.no-auth "no-auth"}
${optionalString cfg.use-auth-secret "use-auth-secret"}
${optionalString (cfg.static-auth-secret != null) ("static-auth-secret=${cfg.static-auth-secret}")}
${optionalString (cfg.static-auth-secret-file != null) ("static-auth-secret=#static-auth-secret#")}
realm=${cfg.realm}
${optionalString cfg.no-udp "no-udp"}
${optionalString cfg.no-tcp "no-tcp"}
${optionalString cfg.no-tls "no-tls"}
${optionalString cfg.no-dtls "no-dtls"}
${optionalString cfg.no-udp-relay "no-udp-relay"}
${optionalString cfg.no-tcp-relay "no-tcp-relay"}
${optionalString (cfg.cert != null) "cert=${cfg.cert}"}
${optionalString (cfg.pkey != null) "pkey=${cfg.pkey}"}
${optionalString (cfg.dh-file != null) ("dh-file=${cfg.dh-file}")}
no-stdout-log
syslog
pidfile=${pidfile}
${optionalString cfg.secure-stun "secure-stun"}
${optionalString cfg.no-cli "no-cli"}
cli-ip=${cfg.cli-ip}
cli-port=${toString cfg.cli-port}
${optionalString (cfg.cli-password != null) ("cli-password=${cfg.cli-password}")}
${cfg.extraConfig}
'';
in {
  options = {
    services.coturn = {
      enable = mkEnableOption (mdDoc "coturn TURN server");
      ...
      no-udp = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Disable UDP client listener";
      };
      no-tcp = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Disable TCP client listener";
      };
      no-tls = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Disable TLS client listener";
      };
      no-dtls = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Disable DTLS client listener";
      };
      no-udp-relay = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Disable UDP relay endpoints";
      };
      no-tcp-relay = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Disable TCP relay endpoints";
      };
      extraConfig = mkOption {
        type = types.lines;
        default = "";
        description = mdDoc "Additional configuration options";
      };
    };
  };
