  ...
  options = {
    services.murmur = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "If enabled, start the Murmur Mumble server.";
      };
      ...
      autobanTime = mkOption {
        type = types.int;
        default = 300;
        description = mdDoc "The amount of time an IP ban lasts (in seconds).";
      };
      ...
      logFile = mkOption {
        ...
        description = mdDoc "Path to the log file for Murmur daemon. Empty means log to journald.";
      };

      welcometext = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Welcome message for connected clients.";
      };

      port = mkOption {
        type = types.port;
        default = 64738;
        description = mdDoc "Ports to bind to (UDP and TCP).";
      };

      hostName = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Host to bind to. Defaults binding on all addresses.";
      };
      ...
      password = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Required password to join server, if specified.";
      };
      ...
      users = mkOption {
        type = types.int;
        default = 100;
        description = mdDoc "Maximum number of concurrent clients allowed.";
      };

      textMsgLength = mkOption {
        type = types.int;
        default = 5000;
        description = mdDoc "Max length of text messages. Set 0 for no limit.";
      };

      imgMsgLength = mkOption {
        type = types.int;
        default = 131072;
        description = mdDoc "Max length of image messages. Set 0 for no limit.";
      };
      ...
      sendVersion = mkOption {
        type = types.bool;
        default = true;
        description = mdDoc "Send Murmur version in UDP response.";
      };
      ...
      registerUrl = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "URL website for your server.";
      };
      ...
      clientCertRequired = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Require clients to authenticate via certificates.";
      };

      sslCert = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Path to your SSL certificate.";
      };

      sslKey = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Path to your SSL key.";
      };

      sslCa = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Path to your SSL CA certificate.";
      };

      extraConfig = mkOption {
        type = types.lines;
        default = "";
        description = mdDoc "Extra configuration to put into murmur.ini.";
      };
      ...
      dbus = mkOption {
        ...
        default = null;
        description = mdDoc "Enable D-Bus remote control. Set to the bus you want Murmur to connect to.";
      };
    };
  };
