      ...
      type = with types; listOf (submodule {
        options = {
          sourcePort = mkOption {
            ...
            example = 8080;
            description = mdDoc "Source port of the external interface; to specify a port range, use a string with a colon (e.g. \"60000:61000\")";
          };

          destination = mkOption {
            type = types.str;
            example = "10.0.0.1:80";
            description = mdDoc "Forward connection to destination ip:port (or [ipv6]:port); to specify a port range, use ip:start-end";
          };
          ...
          proto = mkOption {
            ...
            default = "tcp";
            example = "udp";
            description = mdDoc "Protocol of forwarded connection";
          };
          ...
          loopbackIPs = mkOption {
            ...
            description = mdDoc "Public IPs for NAT reflection; for connections to `loopbackip:sourcePort` from the host itself and from other hosts behind NAT";
          };
        };
      });
      default = [ ];
