...
in {
  options.services.tox-node = {
    enable = mkEnableOption (mdDoc "Tox Node service");

    logType = mkOption {
      type = types.enum [ "Stderr" "Stdout" "Syslog" "None" ];
      default = "Stderr";
      description = mdDoc "Logging implementation.";
    };
    keysFile = mkOption {
      type = types.str;
      default = "${homeDir}/keys";
      description = mdDoc "Path to the file where DHT keys are stored.";
    };
    udpAddress = mkOption {
      type = types.str;
      default = "0.0.0.0:33445";
      description = mdDoc "UDP address to run DHT node.";
    };
    tcpAddresses = mkOption {
      type = types.listOf types.str;
      default = [ "0.0.0.0:33445" ];
      description = mdDoc "TCP addresses to run TCP relay.";
    };
    tcpConnectionLimit = mkOption {
      type = types.int;
      default = 8192;
      description = mdDoc "Maximum number of active TCP connections relay can hold";
    };
    lanDiscovery = mkOption {
      type = types.bool;
      default = true;
      description = mdDoc "Enable local network discovery.";
    };
    threads = mkOption {
      type = types.int;
      default = 1;
      description = mdDoc "Number of threads for execution";
    };
    motd = mkOption {
      type = types.str;
      ...
      description = mdDoc "Message of the day";
    };
  };
