...
in
{
  ...
  options.services.avahi = {
    ...
    ipv4 = mkOption {
      type = types.bool;
      default = true;
      description = mdDoc "Whether to use IPv4.";
    };

    ipv6 = mkOption {
      type = types.bool;
      default = false;
      description = mdDoc "Whether to use IPv6.";
    };
    ...
    wideArea = mkOption {
      type = types.bool;
      default = true;
      description = mdDoc "Whether to enable wide-area service discovery.";
    };

    reflector = mkOption {
      type = types.bool;
      default = false;
      description = mdDoc "Reflect incoming mDNS requests to all allowed network interfaces.";
    };
    ...
    publish = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Whether to allow publishing in general.";
      };

      userServices = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Whether to publish user services. Will set `addresses=true`.";
      };

      addresses = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Whether to register mDNS address records for all local IP addresses.";
      };
      ...
      domain = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Whether to announce the locally used domain name for browsing by other hosts.";
      };
    };
