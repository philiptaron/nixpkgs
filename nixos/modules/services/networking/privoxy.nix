...

{
  ...
  options.services.privoxy = {
    ...
    enable = mkEnableOption (mdDoc "Privoxy, non-caching filtering proxy");
    ...
    settings = mkOption {
      type = types.submodule {
        ...
        options.listen-address = mkOption {
          ...
          description = mdDoc "Pair of address:port the proxy server is listening to.";
        };

        options.enable-edit-actions = mkOption {
          type = types.bool;
          default = false;
          description = mdDoc "Whether the web-based actions file editor may be used.";
        };
