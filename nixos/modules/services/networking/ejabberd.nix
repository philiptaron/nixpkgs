...
let
  ...
  dumps = escapeShellArgs cfg.loadDumps;
...
in {
  ...
  options = {
    ...
    services.ejabberd = {
      ...
      enable = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Whether to enable ejabberd server";
      };
      ...
      user = mkOption {
        type = types.str;
        default = "ejabberd";
        description = mdDoc "User under which ejabberd is ran";
      };

      group = mkOption {
        type = types.str;
        default = "ejabberd";
        description = mdDoc "Group under which ejabberd is ran";
      };

      spoolDir = mkOption {
        type = types.path;
        default = "/var/lib/ejabberd";
        description = mdDoc "Location of the spooldir of ejabberd";
      };

      logsDir = mkOption {
        type = types.path;
        default = "/var/log/ejabberd";
        description = mdDoc "Location of the logfile directory of ejabberd";
      };

      configFile = mkOption {
        type = types.nullOr types.path;
        description = mdDoc "Configuration file for ejabberd in YAML format";
        default = null;
      };
      ...
      ctlConfig = mkOption {
        type = types.lines;
        default = "";
        description = mdDoc "Configuration of ejabberdctl";
      };

      loadDumps = mkOption {
        type = types.listOf types.path;
        default = [];
        description = mdDoc "Configuration dumps that should be loaded on the first startup";
      ...
      imagemagick = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Add ImageMagick to server's path; allows for image thumbnailing";
      };
    };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.ejabberd = {
      ...
      path = [ pkgs.findutils pkgs.coreutils ] ++ optional cfg.imagemagick pkgs.imagemagick;
