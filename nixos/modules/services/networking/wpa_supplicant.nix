...
in {
  options = {
    networking.wireless = {
      enable = mkEnableOption (mdDoc "wpa_supplicant");
      ...
      allowAuxiliaryImperativeNetworks = mkEnableOption (mdDoc "support for imperative & declarative networks") // {
        description = mdDoc ''
      ...
      dbusControlled = mkOption {
        type = types.bool;
        default = length cfg.interfaces < 2;
