...
in
{
  options.services.pomerium = {
    enable = mkEnableOption (mdDoc "the Pomerium authenticating reverse proxy");

    configFile = mkOption {
      type = with types; nullOr path;
      default = null;
      description = mdDoc "Path to Pomerium config YAML. If set, overrides services.pomerium.settings.";
    };
