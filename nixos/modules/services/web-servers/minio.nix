...
in
{
  ...
  options.services.minio = {
    enable = mkEnableOption (mdDoc "Minio Object Storage");

    listenAddress = mkOption {
      default = ":9000";
      type = types.str;
      description = mdDoc "IP address and port of the server.";
    };

    consoleAddress = mkOption {
      default = ":9001";
      type = types.str;
      description = mdDoc "IP address and port of the web UI (console).";
    };

    dataDir = mkOption {
      default = [ "/var/lib/minio/data" ];
      type = types.listOf (types.either types.path types.str);
      description = mdDoc "The list of data directories or nodes for storing the objects. Use one path for regular operation and the minimum of 4 endpoints for Erasure Code mode.";
    };

    configDir = mkOption {
      default = "/var/lib/minio/config";
      type = types.path;
      description = mdDoc "The config directory, for the access keys and other settings.";
    };
    ...
    browser = mkOption {
      default = true;
      type = types.bool;
      description = mdDoc "Enable or disable access to web UI.";
    };
  ...
  config = mkIf cfg.enable {
    ...
    systemd = mkMerge [{
      ...
      ] ++ (map (x: "d '" + x + "' - minio minio - - ") (builtins.filter types.path.check cfg.dataDir));
      ...
      (mkIf (cfg.rootCredentialsFile != null) {
