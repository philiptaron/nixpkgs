...
in
{
  ...
  options.services.molly-brown = {
    ...
    enable = mkEnableOption (mdDoc "Molly-Brown Gemini server");
    ...
    keyPath = mkOption {
      ...
      description = mdDoc "Path to TLS key. See {option}`CertPath`.";
    };

    docBase = mkOption {
      type = types.path;
      example = "/var/lib/molly-brown";
      description = mdDoc "Base directory for Gemini content.";
    };
