...
let
  ...
  isOracle = getName cfg.package == getName pkgs.mysql80;
...

{
  ...
  options = {
    ...
    services.mysql = {
      ...
      enable = mkEnableOption (mdDoc "MySQL server");
      ...
      initialScript = mkOption {
        type = types.nullOr types.path;
        default = null;
        description = mdDoc "A file containing SQL statements to be executed on the first startup. Can be used for granting certain permissions on the database.";
      };
      ...
      replication = {
        role = mkOption {
          ...
          default = "none";
          description = mdDoc "Role of the MySQL server instance.";
        };

        serverId = mkOption {
          type = types.int;
          default = 1;
          description = mdDoc "Id of the MySQL server instance. This number must be unique for each instance.";
        };

        masterHost = mkOption {
          type = types.str;
          description = mdDoc "Hostname of the MySQL master server.";
        };

        slaveHost = mkOption {
          type = types.str;
          description = mdDoc "Hostname of the MySQL slave server.";
        };

        masterUser = mkOption {
          type = types.str;
          description = mdDoc "Username of the MySQL replication user.";
        };

        masterPassword = mkOption {
          type = types.str;
          description = mdDoc "Password of the MySQL replication user.";
        };

        masterPort = mkOption {
          type = types.port;
          default = 3306;
          description = mdDoc "Port number on which the MySQL master server runs.";
        };
      };
    };
