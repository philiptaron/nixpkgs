...
in
{
  ...
  options = {
    services.tarsnap = {
      enable = mkEnableOption (mdDoc "periodic tarsnap backups");
        ...
        type = types.attrsOf (types.submodule ({ config, options, ... }:
          {
            options = {
              ...
              directories = mkOption {
                ...
                default = [];
                description = mdDoc "List of filesystem paths to archive.";
              };
  ...
  config = mkIf gcfg.enable {
    ...
    systemd.services =
      (mapAttrs' (name: cfg: nameValuePair "tarsnap-${name}" {
        ...
        script = let
          tarsnap = ''${getExe gcfg.package} --configfile "/etc/tarsnap/${name}.conf"'';
