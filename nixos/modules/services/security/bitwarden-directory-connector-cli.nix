...
in {
  options.services.bitwarden-directory-connector-cli = {
    ...
    domain = mkOption {
      type = types.str;
      description = mdDoc "The domain the Bitwarden/Vaultwarden is accessible on.";
      example = "https://vaultwarden.example.com";
    };

    user = mkOption {
      type = types.str;
      description = mdDoc "User to run the program.";
      default = "bwdc";
    };
    ...
    interval = mkOption {
      type = types.str;
      default = "*:0,15,30,45";
      description = mdDoc "The interval when to run the connector. This uses systemd's OnCalendar syntax.";
    };
      ...
      type = types.submodule ({
        ...
        options = {
          ...
          ssl = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Whether to use TLS.";
          };
          startTls = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Whether to use STARTTLS.";
          };

          hostname = mkOption {
            type = types.str;
            description = mdDoc "The host the LDAP is accessible on.";
          ...
          port = mkOption {
            type = types.port;
            default = 389;
            description = mdDoc "Port LDAP is accessible on.";
          };

          ad = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Whether the LDAP Server is an Active Directory.";
          };

          pagedSearch = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Whether the LDAP server paginates search results.";
          };

          rootPath = mkOption {
            type = types.str;
            description = mdDoc "Root path for LDAP.";
            example = "dc=example,dc=com";
          };

          username = mkOption {
            type = types.str;
            description = mdDoc "The user to authenticate as.";
      ...
      type = types.submodule ({
        ...
        options = {
          ...
          removeDisabled = mkOption {
            type = types.bool;
            default = true;
            description = mdDoc "Remove users from bitwarden groups if no longer in the ldap group.";
          };
          ...
          overwriteExisting = mkOption {
            ...
            default = false;
            description =
              mdDoc "Remove and re-add users/groups, See https://bitwarden.com/help/user-group-filters/#overwriting-syncs for more details.";
          };

          largeImport = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Enable if you are syncing more than 2000 users/groups.";
          };

          memberAttribute = mkOption {
            type = types.str;
            description = mdDoc "Attribute that lists members in a LDAP group.";
            example = "uniqueMember";
          };

          creationDateAttribute = mkOption {
            type = types.str;
            description = mdDoc "Attribute that lists a user's creation date.";
            example = "whenCreated";
          };
          ...
          useEmailPrefixSuffix = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "If a user has no email address, combine a username prefix with a suffix value to form an email.";
          };
          emailPrefixAttribute = mkOption {
            type = types.str;
            description = mdDoc "The attribute that contains the users username.";
            example = "accountName";
          };
          emailSuffix = mkOption {
            type = types.str;
            description = mdDoc "Suffix for the email, normally @example.com.";
            example = "@example.com";
          };
          ...
          users = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Sync users.";
          };
          userPath = mkOption {
            type = types.str;
            description = mdDoc "User directory, relative to root.";
            default = "ou=users";
          };
          userObjectClass = mkOption {
            type = types.str;
            description = mdDoc "Class that users must have.";
            default = "inetOrgPerson";
          };
          userEmailAttribute = mkOption {
            type = types.str;
            description = mdDoc "Attribute for a users email.";
            default = "mail";
          };
          userFilter = mkOption {
            type = types.str;
            description = mdDoc "LDAP filter for users.";
          ...
          groups = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Whether to sync ldap groups into BitWarden.";
          };
          groupPath = mkOption {
            type = types.str;
            description = mdDoc "Group directory, relative to root.";
            default = "ou=groups";
          };
          groupObjectClass = mkOption {
            type = types.str;
            description = mdDoc "A class that groups will have.";
            default = "groupOfNames";
          };
          groupNameAttribute = mkOption {
            type = types.str;
            description = mdDoc "Attribute for a name of group.";
            default = "cn";
          };
          groupFilter = mkOption {
            type = types.str;
            description = mdDoc "LDAP filter for groups.";
            example = "(cn=sales)";
            default = "";
          };
        };
    ...
    systemd = {
      ...
      services.bitwarden-directory-connector-cli = {
        ...
        preStart = ''
          ...
          ${getExe cfg.package} data-file
          touch /tmp/data.json.tmp
          chmod 600 /tmp/data.json{,.tmp}

          ${getExe cfg.package} config server ${cfg.domain}
          ...
          # final config
          ${getExe cfg.package} config directory 0
          ${getExe cfg.package} config ldap.password --secretfile ${cfg.secrets.ldap}
        '';
        ...
        serviceConfig = {
          ...
          User = "${cfg.user}";
          PrivateTmp = true;
          ExecStart = "${getExe cfg.package} sync";
        };
      };
    };
  };
