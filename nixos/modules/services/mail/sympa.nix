...
in
{
  ...
  options.services.sympa = with types; {
    ...
    enable = mkEnableOption (mdDoc "Sympa mailing list manager");
      ...
      type = attrsOf (submodule ({ name, config, ... }: {
        options = {
          ...
          webLocation = mkOption {
            ...
            default = "/";
            example = "/sympa";
            description = mdDoc "URL path part of the web interface.";
          };
          settings = mkOption {
    ...
    database = {
      ...
      user = mkOption {
        type = nullOr str;
        default = user;
        description = mdDoc "Database user. The system user name is used as a default.";
      };
    ...
    web = {
      enable = mkOption {
        type = bool;
        default = true;
        description = mdDoc "Whether to enable Sympa web interface.";
      };
      ...
      fcgiProcs = mkOption {
        type = ints.positive;
        default = 2;
        description = mdDoc "Number of FastCGI processes to fork.";
      };
    };
    ...
    settingsFile = mkOption {
      type = attrsOf (submodule ({ name, config, ... }: {
        options = {
          enable = mkOption {
            type = bool;
            default = true;
            description = mdDoc "Whether this file should be generated. This option allows specific files to be disabled.";
          };
          text = mkOption {
            default = null;
            type = nullOr lines;
      ...
      '';
      description = mdDoc "Set of files to be linked in {file}`${dataDir}`.";
    };
  };
