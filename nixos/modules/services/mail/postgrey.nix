  ...
  inetSocket = with types; {
    options = {
      addr = mkOption {
        ...
        default = null;
        example = "127.0.0.1";
        description = mdDoc "The address to bind to. Localhost if null";
      };
      port = mkOption {
        type = natural';
        default = 10030;
        description = mdDoc "Tcp port to bind to";
      };
    };
  };
  ...
  unixSocket = with types; {
    options = {
      path = mkOption {
        ...
        description = mdDoc "Path of the unix socket";
      };

      mode = mkOption {
        type = str;
        default = "0777";
        description = mdDoc "Mode of the unix socket";
      };
    };
  };
  ...
  options = {
    services.postgrey = with types; {
      enable = mkOption {
        type = bool;
        default = false;
        description = mdDoc "Whether to run the Postgrey daemon";
      };
      socket = mkOption {
        type = socket;
        default = {
        ...
        description = mdDoc "Socket to bind to";
      };
      greylistText = mkOption {
        type = str;
        default = "Greylisted for %%s seconds";
        description = mdDoc "Response status text for greylisted messages; use %%s for seconds left until greylisting is over and %%r for mail domain of recipient";
      };
      greylistAction = mkOption {
        type = str;
        default = "DEFER_IF_PERMIT";
        description = mdDoc "Response status for greylisted messages (see access(5))";
      };
      greylistHeader = mkOption {
        type = str;
        default = "X-Greylist: delayed %%t seconds by postgrey-%%v at %%h; %%d";
        description = mdDoc "Prepend header to greylisted mails; use %%t for seconds delayed due to greylisting, %%v for the version of postgrey, %%d for the date, and %%h for the host";
      };
      delay = mkOption {
        type = natural;
        default = 300;
        description = mdDoc "Greylist for N seconds";
      };
      maxAge = mkOption {
        type = natural;
        default = 35;
        description = mdDoc "Delete entries from whitelist if they haven't been seen for N days";
      };
      retryWindow = mkOption {
        type = either str natural;
        default = 2;
        example = "12h";
        description = mdDoc "Allow N days for the first retry. Use string with appended 'h' to specify time in hours";
      };
      lookupBySubnet = mkOption {
        type = bool;
        default = true;
        description = mdDoc "Strip the last N bits from IP addresses, determined by IPv4CIDR and IPv6CIDR";
      };
      IPv4CIDR = mkOption {
        type = natural;
        default = 24;
        description = mdDoc "Strip N bits from IPv4 addresses if lookupBySubnet is true";
      };
      IPv6CIDR = mkOption {
        type = natural;
        default = 64;
        description = mdDoc "Strip N bits from IPv6 addresses if lookupBySubnet is true";
      };
      privacy = mkOption {
        type = bool;
        default = true;
        description = mdDoc "Store data using one-way hash functions (SHA1)";
      };
      autoWhitelist = mkOption {
        type = nullOr natural';
        default = 5;
        description = mdDoc "Whitelist clients after successful delivery of N messages";
      };
      whitelistClients = mkOption {
        type = listOf path;
        default = [];
        description = mdDoc "Client address whitelist files (see postgrey(8))";
      };
      whitelistRecipients = mkOption {
        type = listOf path;
        default = [];
        description = mdDoc "Recipient address whitelist files (see postgrey(8))";
      };
    };
  };
