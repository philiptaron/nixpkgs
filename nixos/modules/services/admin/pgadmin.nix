...
in
{
  options.services.pgadmin = {
    enable = mkEnableOption (mdDoc "PostgreSQL Admin 4");

    port = mkOption {
      description = mdDoc "Port for pgadmin4 to run on";
      type = types.port;
      default = 5050;
    };
    ...
    initialEmail = mkOption {
      description = mdDoc "Initial email for the pgAdmin account";
      type = types.str;
    };
    ...
    minimumPasswordLength = mkOption {
      description = mdDoc "Minimum length of the password";
      type = types.int;
      default = 6;
    };
    ...
    emailServer = {
      ...
      address = mkOption {
        description = mdDoc "SMTP server for email delivery";
        type = types.str;
        default = "localhost";
      };
      port = mkOption {
        description = mdDoc "SMTP server port for email delivery";
        type = types.port;
        default = 25;
      };
      useSSL = mkOption {
        description = mdDoc "SMTP server should use SSL";
        type = types.bool;
        default = false;
      };
      useTLS = mkOption {
        description = mdDoc "SMTP server should use TLS";
        type = types.bool;
        default = false;
      };
      username = mkOption {
        description = mdDoc "SMTP server username for email delivery";
    ...
    openFirewall = mkEnableOption (mdDoc "firewall passthrough for pgadmin4");
