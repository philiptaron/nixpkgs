...
in
{
  ...
  options = {
    services.transmission = {
      enable = mkEnableOption (mdDoc "transmission") // {
        description = mdDoc ''
      ...
      settings = mkOption {
        ...
        type = types.submodule {
          ...
          options.download-dir = mkOption {
            ...
            description = mdDoc "Directory where to download torrents.";
          };
          ...
          options.message-level = mkOption {
            ...
            default = 2;
            description = mdDoc "Set verbosity of transmission messages.";
          };
          options.peer-port = mkOption {
            type = types.port;
            default = 51413;
            description = mdDoc "The peer port to listen for incoming connections.";
          };
          ...
          options.peer-port-random-on-start = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc "Randomize the peer port.";
          };
          ...
          options.rpc-port = mkOption {
            type = types.port;
            default = 9091;
            description = mdDoc "The RPC port to listen to.";
          };
          ...
          options.script-torrent-done-filename = mkOption {
            ...
            default = null;
            description = mdDoc "Executable to be run at torrent completion.";
          };
          ...
          options.watch-dir = mkOption {
            ...
            description = mdDoc "Watch a directory for torrent files and add them to transmission.";
          };
          options.watch-dir-enabled = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc ''Whether to enable the
          ...
          options.trash-original-torrent-files = mkOption {
            type = types.bool;
            default = false;
            description = mdDoc ''Whether to delete torrents added from the
              [](#opt-services.transmission.settings.watch-dir).
            '';
          };
        };
      ...
      user = mkOption {
        ...
        description = mdDoc "User account under which Transmission runs.";
      };

      group = mkOption {
        type = types.str;
        default = "transmission";
        description = mdDoc "Group account under which Transmission runs.";
      };
      ...
      openPeerPorts = mkEnableOption (mdDoc "opening of the peer port(s) in the firewall");

      openRPCPort = mkEnableOption (mdDoc "opening of the RPC port in the firewall");

      performanceNetParameters = mkEnableOption (mdDoc "performance tweaks") // {
        description = mdDoc ''
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.transmission = {
      ...
      environment.TRANSMISSION_WEB_HOME = mkIf (cfg.webHome != null) cfg.webHome;

      serviceConfig = {
        # Use "+" because credentialsFile may not be accessible to User= or Group=.
        ExecStartPre = [("+" + pkgs.writeShellScript "transmission-prestart" ''
          set -eu${optionalString (cfg.settings.message-level >= 3) "x"}
