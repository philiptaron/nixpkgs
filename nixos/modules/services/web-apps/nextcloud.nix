...
in {
  ...
  options.services.nextcloud = {
    ...
    hostName = mkOption {
      type = types.str;
      description = mdDoc "FQDN for the nextcloud instance.";
    };
    home = mkOption {
      type = types.str;
      default = "/var/lib/nextcloud";
      description = mdDoc "Storage path of nextcloud.";
    };
    datadir = mkOption {
      type = types.str;
    ...
    https = mkOption {
      type = types.bool;
      default = false;
      description = mdDoc "Use HTTPS for generated links.";
    };
    package = mkOption {
      type = types.package;
      description = mdDoc "Which package to use for the Nextcloud instance.";
    ...
    config = {
      ...
      dbtableprefix = mkOption {
        type = types.nullOr types.str;
        default = null;
        description = mdDoc "Table prefix in Nextcloud's database.";
      };
      adminuser = mkOption {
        type = types.str;
        default = "root";
    ...
    enableImagemagick = mkEnableOption (mdDoc ''
    ...
    settings = mkOption {
      type = types.submodule {
        ...
        options = {
          ...
          "profile.enabled" = mkEnableOption (mdDoc "global profiles") // {
            description = mdDoc ''
    ...
    nginx = {
      recommendedHttpHeaders = mkOption {
        type = types.bool;
        default = true;
        description = mdDoc "Enable additional recommended HTTP response headers";
      };
      hstsMaxAge = mkOption {
    ...
    { systemd.timers.nextcloud-cron = {
      ...
      systemd.services = {
        ...
        in {
          ...
          environment.NC_setup_create_db_user = mkIf (nextcloudGreaterOrEqualThan "26") "false";
        };
        nextcloud-cron = {
          after = [ "nextcloud-setup.service" ];
          ...
          serviceConfig = {
            Type = "oneshot";
            User = "nextcloud";
            ExecCondition = "${getExe phpPackage} -f ${webroot}/occ status -e";
            ExecStart = "${getExe phpPackage} -f ${webroot}/cron.php";
            KillMode = "process";
          };
        };
