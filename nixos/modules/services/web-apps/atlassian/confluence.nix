...

{
  options = {
    services.confluence = {
      enable = mkEnableOption (mdDoc "Atlassian Confluence service");

      user = mkOption {
        type = types.str;
        default = "confluence";
        description = mdDoc "User which runs confluence.";
      };

      group = mkOption {
        type = types.str;
        default = "confluence";
        description = mdDoc "Group which runs confluence.";
      };

      home = mkOption {
        type = types.str;
        default = "/var/lib/confluence";
        description = mdDoc "Home directory of the confluence instance.";
      };
      ...
      proxy = {
        enable = mkEnableOption (mdDoc "proxy support");
      ...
      sso = {
        enable = mkEnableOption (mdDoc "SSO with Atlassian Crowd");

        crowd = mkOption {
          type = types.str;
          example = "http://localhost:8095/crowd";
          description = mdDoc "Crowd Base URL without trailing slash";
        };

        applicationName = mkOption {
          type = types.str;
          example = "jira";
          description = mdDoc "Exact name of this Confluence instance in Crowd";
        };

        applicationPassword = mkOption {
          type = types.nullOr types.str;
          default = null;
          description = mdDoc "Application password of this Confluence instance in Crowd";
        };

        applicationPasswordFile = mkOption {
          type = types.nullOr types.str;
          default = null;
          description = mdDoc "Path to the application password for Crowd of Confluence.";
        };
