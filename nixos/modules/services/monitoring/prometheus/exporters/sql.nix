  ...
  cfgOptions = {
    options = with types; {
      jobs = mkOption {
        ...
        default = { };
        description = mdDoc "An attrset of metrics scraping jobs to run.";
      };
    };
  };
  jobOptions = {
    options = with types; {
      ...
      connections = mkOption {
        type = listOf str;
        description = mdDoc "A list of connection strings of the SQL servers to scrape metrics from";
      };
      startupSql = mkOption {
        type = listOf str;
        default = [];
        description = mdDoc "A list of SQL statements to execute once after making a connection.";
      };
      queries = mkOption {
        type = attrsOf (submodule queryOptions);
        description = mdDoc "SQL queries to run.";
      };
    };
  };
  queryOptions = {
    options = with types; {
      help = mkOption {
        type = nullOr str;
        default = null;
        description = mdDoc "A human-readable description of this metric.";
      };
      labels = mkOption {
        type = listOf str;
        default = [ ];
        description = mdDoc "A set of columns that will be used as Prometheus labels.";
      };
      query = mkOption {
        type = str;
        description = mdDoc "The SQL query to run.";
      };
      values = mkOption {
        type = listOf str;
        description = mdDoc "A set of columns that will be used as values of this metric.";
      };
    };
  };
