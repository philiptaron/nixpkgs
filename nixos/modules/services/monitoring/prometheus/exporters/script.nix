      ...
      type = with types; listOf (submodule {
        options = {
          name = mkOption {
            type = str;
            example = "sleep";
            description = mdDoc "Name of the script.";
          };
          script = mkOption {
            type = str;
            example = "sleep 5";
            description = mdDoc "Shell script to execute when metrics are requested.";
          };
          timeout = mkOption {
            type = nullOr int;
            default = null;
            example = 60;
            description = mdDoc "Optional timeout for the script in seconds.";
          };
        };
      });
