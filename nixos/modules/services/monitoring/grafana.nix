...
let
  ...
  grafanaTypes.datasourceConfig = types.submodule {
    ...
    options = {
      name = mkOption {
        type = types.str;
        description = mdDoc "Name of the datasource. Required.";
      };
      type = mkOption {
        type = types.str;
        description = mdDoc "Datasource type. Required.";
      };
      access = mkOption {
        type = types.enum [ "proxy" "direct" ];
        default = "proxy";
        description = mdDoc "Access mode. proxy or direct (Server or Browser in the UI). Required.";
      };
      uid = mkOption {
        type = types.nullOr types.str;
        default = null;
        description = mdDoc "Custom UID which can be used to reference this datasource in other parts of the configuration, if not specified will be generated automatically.";
      };
      url = mkOption {
        type = types.str;
        default = "localhost";
        description = mdDoc "Url of the datasource.";
      };
      editable = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Allow users to edit datasources from the UI.";
      };
      jsonData = mkOption {
        type = types.nullOr types.attrs;
        default = null;
        description = mdDoc "Extra data for datasource plugins.";
      };
  ...
  grafanaTypes.dashboardConfig = types.submodule {
    ...
    options = {
      name = mkOption {
        type = types.str;
        default = "default";
        description = mdDoc "A unique provider name.";
      };
      type = mkOption {
        type = types.str;
        default = "file";
        description = mdDoc "Dashboard provider type.";
      };
      options.path = mkOption {
        type = types.path;
        description = mdDoc "Path grafana will watch for dashboards. Required when using the 'file' type.";
      };
    };
  };
  ...
  grafanaTypes.notifierConfig = types.submodule {
    options = {
      name = mkOption {
        type = types.str;
        default = "default";
        description = mdDoc "Notifier name.";
      };
      type = mkOption {
        ...
        description = mdDoc "Notifier type.";
      };
      uid = mkOption {
        type = types.str;
        description = mdDoc "Unique notifier identifier.";
      };
      org_id = mkOption {
        type = types.int;
        default = 1;
        description = mdDoc "Organization ID.";
      };
      org_name = mkOption {
        type = types.str;
        default = "Main Org.";
        description = mdDoc "Organization name.";
      };
      is_default = mkOption {
        type = types.bool;
        description = mdDoc "Is the default notifier.";
        default = false;
      };
      send_reminder = mkOption {
        type = types.bool;
        default = true;
        description = mdDoc "Should the notifier be sent reminder notifications while alerts continue to fire.";
      };
      frequency = mkOption {
        type = types.str;
        default = "5m";
        description = mdDoc "How frequently should the notifier be sent reminders.";
      };
      disable_resolve_message = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "Turn off the message that sends when an alert returns to OK.";
      };
      settings = mkOption {
        type = types.nullOr types.attrs;
        default = null;
        description = mdDoc "Settings for the notifier type.";
      };
...
in
{
  ...
  options.services.grafana = {
    enable = mkEnableOption (mdDoc "grafana");

    declarativePlugins = mkOption {
      type = with types; nullOr (listOf path);
      default = null;
      description = mdDoc "If non-null, then a list of packages containing Grafana plugins to install. If set, plugins cannot be manually installed.";
      example = literalExpression "with pkgs.grafanaPlugins; [ grafana-piechart-panel ]";
      # Make sure each plugin is added only once; otherwise building
      # the link farm fails, since the same path is added multiple
      # times.
      apply = x: if isList x then unique x else x;
    };

    package = mkPackageOption pkgs "grafana" { };

    dataDir = mkOption {
      description = mdDoc "Data directory.";
    ...
    settings = mkOption {
      ...
      type = types.submodule {
        ...
        options = {
          paths = {
            plugins = mkOption {
              description = mdDoc "Directory where grafana will automatically scan and look for plugins";
          ...
          server = {
            protocol = mkOption {
              description = mdDoc "Which protocol to listen.";
              default = "http";
            ...
            static_root_path = mkOption {
              description = mdDoc "Root path for static assets.";
          ...
          database = {
            ...
            name = mkOption {
              description = mdDoc "The name of the Grafana database.";
              default = "grafana";
              type = types.str;
            };

            user = mkOption {
              description = mdDoc "The database user (not applicable for `sqlite3`).";
              default = "root";
              type = types.str;
            };
            ...
            max_idle_conn = mkOption {
              description = mdDoc "The maximum number of connections in the idle connection pool.";
              default = 2;
              type = types.int;
            };

            max_open_conn = mkOption {
              description = mdDoc "The maximum number of open connections to the database.";
              default = 0;
              type = types.int;
            };
            ...
            log_queries = mkOption {
              description = mdDoc "Set to `true` to log the sql calls and execution times";
              default = false;
              type = types.bool;
            };
            ...
            ca_cert_path = mkOption {
              description = mdDoc "The path to the CA certificate to use.";
              default = null;
              type = types.nullOr types.str;
            };

            client_key_path = mkOption {
              description = mdDoc "The path to the client key. Only if server requires client authentication.";
              default = null;
              type = types.nullOr types.str;
            };

            client_cert_path = mkOption {
              description = mdDoc "The path to the client cert. Only if server requires client authentication.";
              default = null;
              type = types.nullOr types.str;
            };
            ...
            path = mkOption {
              description = mdDoc "Only applicable to `sqlite3` database. The file path where the database will be stored.";
            ...
            #   description = mdDoc "Set to `true` to add metrics and tracing for database queries.";
            #   default = false;
            #   type = types.bool;
            # };
          };
          ...
          security = {
            disable_initial_admin_creation = mkOption {
              description = mdDoc "Disable creation of admin user on first start of Grafana.";
              default = false;
              type = types.bool;
            };

            admin_user = mkOption {
              description = mdDoc "Default admin username.";
              default = "admin";
              type = types.str;
            };
            ...
            admin_email = mkOption {
              description = mdDoc "The email of the default Grafana Admin, created on startup.";
            ...
            disable_gravatar = mkOption {
              description = mdDoc "Set to `true` to disable the use of Gravatar for user profile images.";
              default = false;
              type = types.bool;
            };
            ...
            disable_brute_force_login_protection = mkOption {
              description = mdDoc "Set to `true` to disable [brute force login protection](https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html#account-lockout).";
              default = false;
              type = types.bool;
            };

            cookie_secure = mkOption {
              description = mdDoc "Set to `true` if you host Grafana behind HTTPS.";
              default = false;
              type = types.bool;
            };
          ...
          smtp = {
            enabled = mkOption {
              description = mdDoc "Whether to enable SMTP.";
              default = false;
              type = types.bool;
            };

            host = mkOption {
              description = mdDoc "Host to connect to.";
              default = "localhost:25";
              type = types.str;
            };

            user = mkOption {
              description = mdDoc "User used for authentication.";
              default = null;
              type = types.nullOr types.str;
            };
            ...
            cert_file = mkOption {
              description = mdDoc "File path to a cert file.";
              default = null;
              type = types.nullOr types.str;
            };

            key_file = mkOption {
              description = mdDoc "File path to a key file.";
              default = null;
              type = types.nullOr types.str;
            };

            skip_verify = mkOption {
              description = mdDoc "Verify SSL for SMTP server.";
              default = false;
              type = types.bool;
            };

            from_address = mkOption {
              description = mdDoc "Address used when sending out emails.";
              default = "admin@grafana.localhost";
              type = types.str;
            };

            from_name = mkOption {
              description = mdDoc "Name to be used as client identity for EHLO in SMTP dialog.";
              default = "Grafana";
              type = types.str;
            };
            ...
            startTLS_policy = mkOption {
              description = mdDoc "StartTLS policy when connecting to server.";
              default = null;
          ...
          users = {
            ...
            allow_org_create = mkOption {
              description = mdDoc "Set to `false` to prohibit users from creating new organizations.";
              default = false;
              type = types.bool;
            };
            ...
            verify_email_enabled = mkOption {
              description = mdDoc "Require email validation before sign up completes.";
              default = false;
              type = types.bool;
            };

            login_hint = mkOption {
              description = mdDoc "Text used as placeholder text on login page for login/username input.";
              default = "email or username";
              type = types.str;
            };

            password_hint = mkOption {
              description = mdDoc "Text used as placeholder text on login page for password input.";
              default = "password";
              type = types.str;
            };

            default_theme = mkOption {
              description = mdDoc "Sets the default UI theme. `system` matches the user's system theme.";
              default = "dark";
              type = types.enum [ "dark" "light" "system" ];
            };

            default_language = mkOption {
              description = mdDoc "This setting configures the default UI language, which must be a supported IETF language tag, such as `en-US`.";
              default = "en-US";
              type = types.str;
            };
            ...
            editors_can_admin = mkOption {
              description = mdDoc "Editors can administrate dashboards, folders and teams they create.";
              default = false;
              type = types.bool;
            };
          ...
          analytics = {
            ...
            feedback_links_enabled = mkOption {
              description = mdDoc "Set to `false` to remove all feedback links from the UI.";
              default = true;
              type = types.bool;
            };
          };
    ...
    provision = {
      enable = mkEnableOption (mdDoc "provision");
        ...
        type = types.submodule {
          options.settings = mkOption {
            ...
            type = types.nullOr (types.submodule {
              options = {
                apiVersion = mkOption {
                  description = mdDoc "Config file version.";
                  default = 1;
                  type = types.int;
                };

                datasources = mkOption {
                  description = mdDoc "List of datasources to insert/update.";
                  default = [ ];
                  type = types.listOf grafanaTypes.datasourceConfig;
                };

                deleteDatasources = mkOption {
                  description = mdDoc "List of datasources that should be deleted from the database.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    options.name = mkOption {
                      description = mdDoc "Name of the datasource to delete.";
                      type = types.str;
                    };

                    options.orgId = mkOption {
                      description = mdDoc "Organization ID of the datasource to delete.";
                      type = types.int;
                    };
                  });
                };
          ...
          options.settings = mkOption {
            ...
            type = types.nullOr (types.submodule {
              ...
              options.providers = mkOption {
                description = mdDoc "List of dashboards to insert/update.";
                default = [ ];
      ...
      notifiers = mkOption {
        description = mdDoc "Grafana notifier configuration.";
        default = [ ];
          ...
          settings = mkOption {
            ...
            type = types.nullOr (types.submodule {
              options = {
                ...
                groups = mkOption {
                  description = mdDoc "List of rule groups to import or update.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    freeformType = provisioningSettingsFormat.type;

                    options.name = mkOption {
                      description = mdDoc "Name of the rule group. Required.";
                      type = types.str;
                    };

                    options.folder = mkOption {
                      description = mdDoc "Name of the folder the rule group will be stored in. Required.";
                      type = types.str;
                    };

                    options.interval = mkOption {
                      description = mdDoc "Interval that the rule group should be evaluated at. Required.";
                      type = types.str;
                    };
                  });
                };
                ...
                deleteRules = mkOption {
                  description = mdDoc "List of alert rule UIDs that should be deleted.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    options.orgId = mkOption {
                      description = mdDoc "Organization ID, default = 1";
                      default = 1;
                      type = types.int;
                    };

                    options.uid = mkOption {
                      description = mdDoc "Unique identifier for the rule. Required.";
                      type = types.str;
                    };
                  });
                };
            ...
            type = types.nullOr (types.submodule {
              options = {
                ...
                contactPoints = mkOption {
                  description = mdDoc "List of contact points to import or update.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    freeformType = provisioningSettingsFormat.type;

                    options.name = mkOption {
                      description = mdDoc "Name of the contact point. Required.";
                      type = types.str;
                    };
                  });
                };
                ...
                deleteContactPoints = mkOption {
                  description = mdDoc "List of receivers that should be deleted.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    options.orgId = mkOption {
                      description = mdDoc "Organization ID, default = 1.";
                    ...
                    options.uid = mkOption {
                      description = mdDoc "Unique identifier for the receiver. Required.";
                      type = types.str;
                    };
                  });
                };
            ...
            type = types.nullOr (types.submodule {
              options = {
                ...
                resetPolicies = mkOption {
                  description = mdDoc "List of orgIds that should be reset to the default policy.";
                  default = [ ];
          ...
          settings = mkOption {
            ...
            type = types.nullOr (types.submodule {
              options = {
                ...
                templates = mkOption {
                  description = mdDoc "List of templates to import or update.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    freeformType = provisioningSettingsFormat.type;

                    options.name = mkOption {
                      description = mdDoc "Name of the template, must be unique. Required.";
                      type = types.str;
                    };

                    options.template = mkOption {
                      description = mdDoc "Alerting with a custom text template";
                      type = types.str;
                    };
                  });
                };
          ...
          settings = mkOption {
            ...
            type = types.nullOr (types.submodule {
              options = {
                ...
                muteTimes = mkOption {
                  description = mdDoc "List of mute time intervals to import or update.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    freeformType = provisioningSettingsFormat.type;

                    options.name = mkOption {
                      description = mdDoc "Name of the mute time interval, must be unique. Required.";
                      type = types.str;
                    };
                  });
                };
                ...
                deleteMuteTimes = mkOption {
                  description = mdDoc "List of mute time intervals that should be deleted.";
                  default = [ ];
                  type = types.listOf (types.submodule {
                    options.orgId = mkOption {
                      description = mdDoc "Organization ID, default = 1.";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.grafana = {
      ...
      after = [ "networking.target" ] ++ optional usePostgresql "postgresql.service" ++ optional useMysql "mysql.service";
      script = ''
      ...
      serviceConfig = {
        ...
        # Hardening
        AmbientCapabilities = mkIf (cfg.settings.server.http_port < 1024) [ "CAP_NET_BIND_SERVICE" ];
        ...
        SystemCallFilter = [
          "@system-service"
          "~@privileged"
        ] ++ optionals (cfg.settings.server.protocol == "socket") [ "@chown" ];
        UMask = "0027";
      };
      preStart = ''
