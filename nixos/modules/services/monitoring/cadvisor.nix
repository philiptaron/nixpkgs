...
in
{
  options = {
    services.cadvisor = {
      enable = mkEnableOption (mdDoc "Cadvisor service");

      listenAddress = mkOption {
        default = "127.0.0.1";
        type = types.str;
        description = mdDoc "Cadvisor listening host";
      };

      port = mkOption {
        default = 8080;
        type = types.port;
        description = mdDoc "Cadvisor listening port";
      };
      ...
      storageDriver = mkOption {
        ...
        example = "influxdb";
        description = mdDoc "Cadvisor storage driver.";
      };

      storageDriverHost = mkOption {
        default = "localhost:8086";
        type = types.str;
        description = mdDoc "Cadvisor storage driver host.";
      };

      storageDriverDb = mkOption {
        default = "root";
        type = types.str;
        description = mdDoc "Cadvisord storage driver database name.";
      };

      storageDriverUser = mkOption {
        default = "root";
        type = types.str;
        description = mdDoc "Cadvisor storage driver username.";
      };
      ...
      storageDriverSecure = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "Cadvisor storage driver, enable secure communication.";
      };
