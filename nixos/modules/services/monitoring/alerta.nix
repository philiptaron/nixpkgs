...
in
{
  options.services.alerta = {
    enable = mkEnableOption (mdDoc "alerta");

    port = mkOption {
      type = types.port;
      default = 5000;
      description = mdDoc "Port of Alerta";
    };
    ...
    logDir = mkOption {
      type = types.path;
      description = mdDoc "Location where the logfiles are stored";
      default = "/var/log/alerta";
    };

    databaseUrl = mkOption {
      type = types.str;
      description = mdDoc "URL of the MongoDB or PostgreSQL database to connect to";
      default = "mongodb://localhost";
    };

    databaseName = mkOption {
      type = types.str;
      description = mdDoc "Name of the database instance to connect to";
      default = "monitoring";
    };

    corsOrigins = mkOption {
      type = types.listOf types.str;
      description = mdDoc "List of URLs that can access the API for Cross-Origin Resource Sharing (CORS)";
      default = [ "http://localhost" "http://localhost:5000" ];
    };

    authenticationRequired = mkOption {
      type = types.bool;
      description = mdDoc "Whether users must authenticate when using the web UI or command-line tool";
      default = false;
    };

    signupEnabled = mkOption {
      type = types.bool;
      description = mdDoc "Whether to prevent sign-up of new users via the web UI";
      default = true;
    };

    extraConfig = mkOption {
      description = mdDoc "These lines go into alertad.conf verbatim.";
      default = "";
      type = types.lines;
    };
  };
