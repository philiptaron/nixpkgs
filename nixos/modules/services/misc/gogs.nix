  ...
  options = {
    services.gogs = {
      enable = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "Enable Go Git Service.";
      };

      useWizard = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "Do not generate a configuration and use Gogs' installation wizard instead. The first registered user will be administrator.";
      };

      stateDir = mkOption {
        default = "/var/lib/gogs";
        type = types.str;
        description = mdDoc "Gogs data directory.";
      };

      user = mkOption {
        type = types.str;
        default = "gogs";
        description = mdDoc "User account under which Gogs runs.";
      };

      group = mkOption {
        type = types.str;
        default = "gogs";
        description = mdDoc "Group account under which Gogs runs.";
      };
      ...
      rootUrl = mkOption {
        ...
        description = mdDoc "Full public URL of Gogs server.";
      };

      httpAddress = mkOption {
        type = types.str;
        default = "0.0.0.0";
        description = mdDoc "HTTP listen address.";
      };

      httpPort = mkOption {
        type = types.port;
        default = 3000;
        description = mdDoc "HTTP listen port.";
      };
      ...
      extraConfig = mkOption {
        type = types.str;
        default = "";
        description = mdDoc "Configuration lines appended to the generated Gogs configuration file.";
      };
    };
  };
