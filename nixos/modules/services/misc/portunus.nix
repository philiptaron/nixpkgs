...
in
{
  options.services.portunus = {
    enable = mkEnableOption (mdDoc "Portunus, a self-contained user/group management and authentication service for LDAP");

    domain = mkOption {
      type = types.str;
      example = "sso.example.com";
      description = mdDoc "Subdomain which gets reverse proxied to Portunus webserver.";
    };
    ...
    seedSettings = mkOption {
      type = with types; nullOr (attrsOf (listOf (attrsOf anything)));
      default = null;
      description = mdDoc ''
    ...
    stateDir = mkOption {
      ...
      description = mdDoc "Path where Portunus stores its state.";
    };

    user = mkOption {
      type = types.str;
      default = "portunus";
      description = mdDoc "User account under which Portunus runs its webserver.";
    };

    group = mkOption {
      type = types.str;
      default = "portunus";
      description = mdDoc "Group account under which Portunus runs its webserver.";
    };
    ...
    dex = {
        ...
        type = types.listOf (types.submodule {
          options = {
            callbackURL = mkOption {
              type = types.str;
              description = mdDoc "URL where the OIDC client should redirect";
            };
            id = mkOption {
              type = types.str;
              description = mdDoc "ID of the OIDC client";
            };
          };
        });
        default = [ ];
      ...
      port = mkOption {
        type = types.port;
        default = 5556;
        description = mdDoc "Port where dex should listen on.";
      };
    };
    ...
    ldap = {
      package = mkOption {
        ...
        defaultText = literalExpression "pkgs.openldap.override { libxcrypt = pkgs.libxcrypt-legacy; }";
        description = mdDoc "The OpenLDAP package to use.";
      };
      ...
      user = mkOption {
        type = types.str;
        default = "openldap";
        description = mdDoc "User account under which Portunus runs its LDAP server.";
      };

      group = mkOption {
        type = types.str;
        default = "openldap";
        description = mdDoc "Group account under which Portunus runs its LDAP server.";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    services = {
      ...
      portunus.seedPath = mkIf (cfg.seedSettings != null) (pkgs.writeText "seed.json" (builtins.toJSON cfg.seedSettings));
    };
