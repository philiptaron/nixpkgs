...
in
{
  options.services.irkerd = {
    enable = mkOption {
      description = mdDoc "Whether to enable irker, an IRC notification daemon.";
      default = false;
      type = types.bool;
    };

    openPorts = mkOption {
      description = mdDoc "Open ports in the firewall for irkerd";
      default = false;
      type = types.bool;
    };
    ...
    nick = mkOption {
      default = "irker";
      type = types.str;
      description = mdDoc "Nick to use for irker";
    };
  };
