...
in
{
  options.hardware.fancontrol = {
    enable = mkEnableOption (mdDoc "software fan control (requires fancontrol.config)");

    config = mkOption {
      type = types.lines;
      description = mdDoc "Required fancontrol configuration file content. See {manpage}`pwmconfig(8)` from the lm_sensors package.";
      example = ''
