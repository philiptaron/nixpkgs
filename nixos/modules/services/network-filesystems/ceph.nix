...
let
  ...
  makeService = daemonType: daemonId: clusterName: ceph:
    ...
    startLimitBurst =
      if daemonType == "osd" then 30 else if elem daemonType ["mgr" "mds"] then 3 else 5;
...
in
{
  options.services.ceph = {
    ...
    enable = mkEnableOption (mdDoc "Ceph global configuration");
    ...
    mgr = {
      enable = mkEnableOption (mdDoc "Ceph MGR daemon");
      daemons = mkOption {
    ...
    mon = {
      enable = mkEnableOption (mdDoc "Ceph MON daemon");
      daemons = mkOption {
        type = with types; listOf str;
        default = [];
        example = [ "name1" "name2" ];
    ...
    osd = {
      enable = mkEnableOption (mdDoc "Ceph OSD daemon");
      daemons = mkOption {
        type = with types; listOf str;
        default = [];
        example = [ "name1" "name2" ];
    ...
    mds = {
      enable = mkEnableOption (mdDoc "Ceph MDS daemon");
      daemons = mkOption {
        type = with types; listOf str;
        default = [];
        example = [ "name1" "name2" ];
    ...
    rgw = {
      enable = mkEnableOption (mdDoc "Ceph RadosGW daemon");
      package = mkPackageOption pkgs "ceph" { };
      daemons = mkOption {
        type = with types; listOf str;
        default = [];
    ...
    client = {
      enable = mkEnableOption (mdDoc "Ceph client configuration");
      extraConfig = mkOption {
