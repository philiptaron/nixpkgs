...
in
{
  ...
  options = {
    security.duosec = {
      ssh.enable = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "If enabled, protect SSH logins with Duo Security.";
      };

      pam.enable = mkOption {
        type = types.bool;
        default = false;
        description = mdDoc "If enabled, protect logins with Duo Security using PAM support.";
      };

      integrationKey = mkOption {
        type = types.str;
        description = mdDoc "Integration key.";
      };
      ...
      host = mkOption {
        type = types.str;
        description = mdDoc "Duo API hostname.";
      };
  ...
  config = mkIf (cfg.ssh.enable || cfg.pam.enable) {
    ...
    systemd.services.login-duo = mkIf cfg.ssh.enable {
    ...
    systemd.services.pam-duo = mkIf cfg.ssh.enable {
      wantedBy = [ "sysinit.target" ];
      before = [ "sysinit.target" "shutdown.target" ];
      conflicts = [ "shutdown.target" ];
      unitConfig.DefaultDependencies = false;
