...
let
  ...
  pamOpts = { config, name, ... }: let cfg = config; in let config = parentConfig; in {
    ...
    imports = [
      (mkRenamedOptionModule [ "enableKwallet" ] [ "kwallet" "enable" ])
    ];
    ...
    options = {
      ...
      name = mkOption {
        example = "sshd";
        type = types.str;
        description = mdDoc "Name of the PAM service.";
      };
      ...
      showMotd = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "Whether to show the message of the day.";
      };
      ...
      updateWtmp = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "Whether to update {file}`/var/log/wtmp`.";
      };

      logFailures = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "Whether to log authentication failures in {file}`/var/log/faillog`.";
      };
      ...
      sssdStrictAccess = mkOption {
        default = false;
        type = types.bool;
        description = mdDoc "enforce sssd access control";
      };
      ...
      failDelay = {
        ...
        delay = mkOption {
          ...
          type = types.int;
          example = 1000000;
          description = mdDoc "The delay time (in microseconds) on failure.";
        };
      };
      ...
      text = mkOption {
        ...
        description = mdDoc "Contents of the PAM service file.";
      };
      ...
      in {
        ...
        auth = autoOrderRules ([
          ...
          { name = "ssh_agent_auth"; enable = config.security.pam.sshAgentAuth.enable && cfg.sshAgentAuth; control = "sufficient"; modulePath = "${pkgs.pam_ssh_agent_auth}/libexec/pam_ssh_agent_auth.so"; settings = {
            file = concatStringsSep ":" config.security.pam.sshAgentAuth.authorizedKeysFiles;
          }; }
  ...
  limitsType = with types; listOf (submodule ({ ... }: {
    options = {
      domain = mkOption {
        description = mdDoc "Username, groupname, or wildcard this limit applies to";
        example = "@wheel";
        type = str;
      };

      type = mkOption {
        description = mdDoc "Type of this limit";
        type = enum [ "-" "hard" "soft" ];
        default = "-";
      };

      item = mkOption {
        description = mdDoc "Item this limit applies to";
        type = enum [
          "core"
          "data"
          "fsize"
      ...
      value = mkOption {
        description = mdDoc "Value of this limit";
        type = oneOf [ str int ];
      };
    };
  }));
...

{
  ...
  options = {
    ...
    security.pam.enableOTPW = mkEnableOption (mdDoc "the OTPW (one-time password) PAM module");
    ...
    security.pam.enableEcryptfs = mkEnableOption (mdDoc "eCryptfs PAM module (mounting ecryptfs home directory on login)");
    security.pam.enableFscrypt = mkEnableOption (mdDoc ''
    ...
    users.motd = mkOption {
      ...
      type = types.nullOr types.lines;
      description = mdDoc "Message of the day shown to users when they log in.";
    };
    ...
    users.motdFile = mkOption {
      ...
      example = "/etc/motd";
      type = types.nullOr types.path;
      description = mdDoc "A file containing the message of the day shown to users when they log in.";
    };
  };
