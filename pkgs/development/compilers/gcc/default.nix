...
let
  ...
  is7  = majorVersion == "7";
  is6  = majorVersion == "6";
  is49 = majorVersion == "4" && minor version == "9";
  is48 = majorVersion == "4" && minor version == "8";
in
  ...
  inherit version;
    ...
    callFile = callPackageWith ({
      ...
      inherit
        ...
        crossNameAddon
      ;
      # inherit generated with 'nix eval --json --impure --expr "with import ./. {}; attrNames (functionArgs gcc${majorVersion}.cc.override)" | jq '.[]' --raw-output'
      inherit
        binutils
        buildPackages
        cloog
...
pipe ((callFile ./common/builder.nix {}) ({
  ...
  env = mapAttrs (_: v: toString v) ({
    ...
    #
    # Likewise, the LTO code doesn't find z
    #
