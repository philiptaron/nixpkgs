{
  ...
  # List representing a nixos option path (e.g. ['console' 'enable']), or a prefix of such a path
  # (e.g. ['console']), or a string representing the same (e.g. 'console.enable')
  path,
...
let
  ...
  # Helper that maps `f` on subslices starting when `predStart x` and ending when `predEnd x`
  # (no support for nested occurrences)
  flatMapSlices =
  ...
  # Tries to invert showOption, taking a written-out option name and splitting it into its parts.
  readOption =
    str:
    let
  ...
  # Like 'mapAttrsRecursiveCond' but handling errors in the attrset tree as leaf nodes
  # (which means `f` is expected to handle shallow errors)
  ...
  # Traverse the option tree along `path` from `root`, returning the option or attrset at the given
  # location
  optionByPath =
    path: root:
    let
